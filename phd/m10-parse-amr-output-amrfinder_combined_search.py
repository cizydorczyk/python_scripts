...
for isolate in isolate_list:
    ...
    with open(amrfinderplus_file, "r") as infile1:
        for line in infile1:
            if not line.startswith("Name"):
                gene = line.strip().split("\t")[6].lower()
                gene_cov = line.strip().split("\t")[16]
                gene_pid = line.strip().split("\t")[17]
                gene_id = f"A_{gene}_{gene_cov}_{gene_pid}"
                if gene not in amrfinder_genes:
                    amrfinder_genes[gene] = [gene_id]
                elif gene in amrfinder_genes:
                    amrfinder_genes[gene].append(gene_id)
    ...
    with open(cardrgi_file, "r") as infile2:
        for line in infile2:
            if not line.startswith("ORF_ID"):
                gene = line.strip().split("\t")[8].lower()
                gene_cov = line.strip().split("\t")[20]
                gene_pid = line.strip().split("\t")[9]
                gene_id = f"C_{gene}_{gene_cov}_{gene_pid}"
                if gene not in cardrgi_genes:
                    cardrgi_genes[gene] = [gene_id]
                elif gene in cardrgi_genes:
                    cardrgi_genes[gene].append(gene_id)
    ...
    with open(resfinder_file, "r") as infile3:
        for line in infile3:
            if not line.startswith("Resistance"):
                gene = line.strip().split("\t")[0].lower()
                gene_cov = line.strip().split("\t")[3]
                gene_pid = line.strip().split("\t")[1]
                gene_id = f"R_{gene}_{gene_cov}_{gene_pid}"
                if gene not in resfinder_genes:
                    resfinder_genes[gene] = [gene_id]
                elif gene in resfinder_genes:
                    resfinder_genes[gene].append(gene_id)

    for gene in output_dict:
        key = ""
        if gene in amrfinder_genes:
            key = key + "_" + "-".join(amrfinder_genes[gene])
        if gene in cardrgi_genes:
            key = key + "_" + "-".join(cardrgi_genes[gene])
        if gene in resfinder_genes:
            key = key + "_" + "-".join(resfinder_genes[gene])
